    ...
    >
      ...
      >
        <div className="w-full md:w-1/3 mb-4 md:mb-0">
          <p className="text font-primary text-lg font-medium tracking-wide text-white">News</p>
        </div>
        <div className="w-full md:w-2/3 xl:w-3/4 flex flex-col items-start text-left">
          ...
          <p
            ...
            style={{ opacity: 0, transform: 'translateY(20px)' }}
          >
            With 36 projects under construction around the world and 22 more in design development, our insatiable appetite for research, and myriad collaborations both physical and virtual, there is always something exciting happening at ABH. See the latest.
          </p>
        </div>
      </div>
      ...
      <div className="relative w-full">
        {/* Navigation Arrows */}
        <div className={`absolute left-6 top-1/2 transform -translate-y-1/2 z-20 md:flex ${currentIndex === 0 ? 'opacity-50 pointer-events-none' : 'pointer-events-auto'}`}>
          <div
            onClick={prevSlide}
        ...
        <div className={`absolute right-6 top-1/2 transform -translate-y-1/2 z-20 md:flex ${currentIndex === newsItems.length - 1 ? 'opacity-50 pointer-events-none' : 'pointer-events-auto'}`}>
          <div
            onClick={nextSlide}
            className="w-24 h-24 rounded-full border border-white/40 bg-black/5 backdrop-blur-md flex items-center justify-center text-white cursor-pointer hover:bg-white/10 transition-colors"
          >
        ...
        <div
          ref={carouselRef}
          className="flex flex-row items-start w-full px-6 md:px-12 overflow-hidden"
          style={{ opacity: 0 }}
        >
          <div className="flex flex-row items-start gap-6 pb-8" style={{ width: 'max-content' }}>

            {newsItems.map((item, index) => (
              <div key={item.id} className="carousel-item w-[85vw] md:w-[90vw] shrink-0" style={{ transform: 'translateX(0)' }}>
                  ...
                  <div className="absolute bottom-0 left-0 w-full p-6 md:p-8 flex flex-col z-20 shrink-0 pointer-events-auto group-hover:bg-white transition-colors duration-500 rounded-lg">
                    ...
                    <div className="flex flex-row w-full justify-between items-center mb-6 text-sm font-medium relative">
                      ...
                      <div className="absolute left-1/2 transform -translate-x-1/2 text-white/60 group-hover:text-gray-400 transition-colors duration-500">
                        <span className="text-white group-hover:text-black transition-colors duration-500">0{index + 1}</span> / {newsItems.length < 10 ? `0${newsItems.length}` : newsItems.length}
                      </div>
